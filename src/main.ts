...
class Router {
  ...
  private initializeRouting(): void {
    ...
    const navLinks = document.querySelectorAll('.nav-link, .navbar-logo');
    navLinks.forEach(link => {
      link.addEventListener('click', (e) => {
        e.preventDefault();
        const targetPage = (link as HTMLElement).dataset.page;
        if (targetPage) {
    ...
    if (mobileMenuToggle && navbarNav) {
      ...
      const mobileNavLinks = navbarNav.querySelectorAll('.nav-link');
  ...
  private showPage(page: string, updateHistory: boolean = true): void {
    // Hide all pages
